export class ZoteroService {
	...
	async getPDFPath(itemKey: string, libraryId: string): Promise<string> {
		...
		if (!response.ok) {
			throw new Error(`BBT JSON-RPC error ${response.status}: ${response.statusText}`);
		}
		...
		const attachments: Array<{ path?: string; contentType?: string }> = data.result ?? [];
		const pdf = attachments.find((a) => a.contentType === 'application/pdf' && a.path);

		if (!pdf?.path) {
			throw new Error(`No PDF attachment found for item key "${itemKey}" (library ${libraryId}).`);
		}
